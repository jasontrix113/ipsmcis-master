<?php 
    ...
    foreach($ids as $id) {
        ...
        foreach($array as $arr) {
            ...
            (`applicantsid`, `courseid`, `traveldest`, `country`, `date_encoded`, `month`, `year`) values('$userid', '$id', '0', '$country', '$now', '$monthName', '$year')");
            $insertSchool->execute();
            date_default_timezone_set("Asia/Manila");
            $dateNow = date('Y-m-d');
            $timeNow = date('h:i:s A');
            $insertLog = $dbh->prepare("INSERT INTO logs(`log_description`, `date`, `time`) VALUES('Added New Match Student','".$dateNow."','".$timeNow."')");
            $insertLog->execute(); 
        }
    }
