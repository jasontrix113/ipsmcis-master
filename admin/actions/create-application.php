    ...
    if(isset($_POST['submit'])) {
        ...
        if($rows == 0){
            ...
            if($file_1 == true && $file_2 == true && $file_3 == true && $file_4 == true ){
                $insertApplication = $dbh->prepare("INSERT INTO `visa_application` (`applicantsid`, `counter`, `category`, `title`, `firstname`, `middlename`, `lastname`, `gender`, `dateofbirth`, `birthplace`, `nationality`,`civilstatus`, `address`, `passport`,`home`, `mobile`, `emailaddress`, `status`, `datecreated`,`countryofpassport`,`dateofissue`,`placeofissue`, `nso`, `ielts`, `nbi`, `picture`,`month`,`year`) VALUES ('".$category_id."', '".$counter0."', '".$category."', '".$title."', '".$_POST['FirstName']."', '".$_POST['MiddleName']."', '".$_POST['LastName']."', '".$gender."', '".$birthday."', '".$birthplace."', '".$nationality."', '".$civilstatus."', '".$address."', '".$passport."','".$homeNumber."', '".$mobileNumber."', '".$email."', 'Pending', '".date("Y-m-d")."', '".$cpassport."', '".$dissue."', '".$pissue."', '".$tempfile01."', '".$tempfile02."', '".$tempfile03."', '".$tempfile04."', '".$monthName."', '".$year."')");
                $insertApplication->execute();
                $insertUser = $dbh->prepare("INSERT INTO `users`(`fullname`, `username`, `password`, `usertype`, `disable`, `applicantsid`, `month`, `year`) VALUES('".$name."','".$username."', '".$password."', 'Client', '0', '".$category_id."', '".$monthName."', '".$year."')");
                $insertUser->execute(); 
                date_default_timezone_set("Asia/Manila");
                $dateNow = date('Y-m-d');
                $timeNow = date('h:i:s A');
                $insertLog = $dbh->prepare("INSERT INTO logs(`log_description`, `date`, `time`) VALUES('".$_POST['FirstName']." ".$_POST['MiddleName']." ".$_POST['LastName']." - Visa Application Added','".$dateNow."','".$timeNow."')");
                $insertLog->execute(); 
