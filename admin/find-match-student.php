            ...
            <div class="card m-b-30">
                ...
                <div class="card-body">
                                ...
                                if(isset($_POST['submit'])) {
                                    ...
                                    if($currency == 'PHP') {
                                        if($tuitionFrom == "" && $tuitionTo == "") {
                                            $match = $dbh->prepare("SELECT * FROM `school_details` INNER JOIN `courses` ON `school_details`.`school_id`=`courses`.`school_id` WHERE `school_country`='".$country."' AND `category` = '".$category."' order by courses.tuition");
                                        }else {
                                            $match = $dbh->prepare("SELECT * FROM `school_details` INNER JOIN `courses` ON `school_details`.`school_id`=`courses`.`school_id` WHERE `school_country`='".$country."' AND `category` = '".$category."' AND 
                                            courses.tuition BETWEEN '$tuitionFrom' AND '$tuitionTo' order by courses.tuition");
                                        }
                    ...
                    <div class="submit-btn" style="margin: 0 auto; text-align: center !important;">
                        ...
                        <button class="btn-select btn btn-primary center" id="btn-select-me" style="text-align: center; margin: 0 auto;">Select Schools</button>
                    </div>
                </div>
            </div>
        </div>
